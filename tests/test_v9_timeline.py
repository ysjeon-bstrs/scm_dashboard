...
def sample_snapshot() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "resource_code": ["BA00021", "BA00021", "BA00021", "BA00021", "BA00021", "BA00021"],
...
def test_prepare_snapshot(sample_snapshot):
    ...
    assert all(col in result.data.columns for col in ["date", "center", "resource_code", "stock_qty"])
...
def test_timeline_builder_empty_snapshot():
    ...
    empty_snapshot = prepare_snapshot(pd.DataFrame({
        "date": [],
        "center": [],
        "resource_code": [],
        "stock_qty": []
...
def test_timeline_builder_basic(sample_snapshot, sample_moves, timeline_context):
    ...
    assert all(col in bundle.center_lines.columns for col in ["date", "center", "resource_code", "stock_qty"])
...
def test_timeline_builder_wip_handling(sample_moves, timeline_context):
    ...
    builder = TimelineBuilder(context)
    snapshot = prepare_snapshot(pd.DataFrame({
