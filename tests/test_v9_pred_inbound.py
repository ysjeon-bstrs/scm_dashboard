    ...
    return pd.DataFrame(
        {
            ...
            "onboard_date": pd.to_datetime(
                ["2024-01-01", "2024-01-05", "2023-12-15", "2024-01-10", "2024-01-08"]
            ),
            "arrival_date": pd.to_datetime(
                ["2024-01-15", "2024-01-08", pd.NaT, "2024-01-20", pd.NaT]
            ),
            "eta_date": pd.to_datetime([pd.NaT, pd.NaT, pd.NaT, "2024-01-21", pd.NaT]),
            "inbound_date": pd.to_datetime(
                [pd.NaT, "2024-01-12", pd.NaT, pd.NaT, pd.NaT]
            ),
            "event_date": pd.to_datetime(
                [pd.NaT, pd.NaT, "2024-01-25", pd.NaT, "2024-02-05"]
            ),
        }
    )
...
def _calculate_pred_inbound_reference(
    moves: pd.DataFrame, today: pd.Timestamp, lag_days: int
) -> pd.DataFrame:
    """
    ...
    for col in ["arrival_date", "eta_date", "inbound_date", "event_date"]:
        if col in moves_view.columns:
            moves_view[col] = pd.to_datetime(
                moves_view[col], errors="coerce"
            ).dt.normalize()
    ...
    if wip_mask.any() and "event_date" in moves_view.columns:
        event_series = pd.to_datetime(
            moves_view.get("event_date"), errors="coerce"
        ).dt.normalize()
