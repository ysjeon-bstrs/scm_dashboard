...
def test_normalize_snapshot_stock_qty_always_float():
    ...
    assert (
        result["stock_qty"].dtype == "float64"
...
def test_validate_timeline_inputs_missing_snapshot_columns():
    ...
    with pytest.raises(ValidationError, match="필요한 컬럼이 없습니다"):
        validate_timeline_inputs(
            snapshot, moves, pd.Timestamp("2024-01-01"), pd.Timestamp("2024-01-10")
        )
...
def test_filter_by_centers():
    """센터 필터링"""
    df = pd.DataFrame({"center": ["태광KR", "AMZUS", "상해CN"], "qty": [100, 200, 150]})
...
def test_filter_by_skus():
    """SKU 필터링"""
    df = pd.DataFrame(
        {"resource_code": ["BA00021", "BA00022", "BA00023"], "qty": [100, 200, 150]}
    )
