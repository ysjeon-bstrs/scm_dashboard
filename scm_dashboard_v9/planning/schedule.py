    ...
    *,
    ...
    for col in ["carrier_mode", "inbound_date", "arrival_date", "eta_date", "event_date"]:
        if col not in moves_out.columns:
            if "date" in col:
                moves_out[col] = pd.Series(pd.NaT, index=moves_out.index, dtype="datetime64[ns]")
            else:
    ...
    for col in ["arrival_date", "eta_date", "inbound_date", "event_date"]:
        if col in moves_out.columns:
            moves_out[col] = pd.to_datetime(moves_out[col], errors="coerce").dt.normalize()
    ...
    if wip_mask.any():
        ...
        if isinstance(event_series, pd.Series):
            event_normalized = pd.to_datetime(event_series, errors="coerce").dt.normalize()
    ...
    if isinstance(arrival_series, pd.Series) and isinstance(eta_series, pd.Series):
        arrival_normalized = pd.to_datetime(arrival_series, errors="coerce").dt.normalize()
        ...
        effective_arrival = pd.to_datetime(arrival_series, errors="coerce").dt.normalize()
    elif isinstance(eta_series, pd.Series):
        effective_arrival = pd.to_datetime(eta_series, errors="coerce").dt.normalize()
    else:
        effective_arrival = pd.Series(pd.NaT, index=moves_out.index, dtype="datetime64[ns]")
    ...
    if mask_eta.any():
        ...
        if past_eta.any():
            pred_inbound.loc[past_eta] = today_norm + pd.Timedelta(days=int(past_arrival_buffer_days))
