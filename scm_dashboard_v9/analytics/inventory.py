...
def pivot_inventory_cost_from_raw(
    ...
    """스냅샷 raw 데이터로부터 센터별 재고자산 피벗 테이블을 생성합니다.

    Args:
        snap_raw: 스냅샷 raw DataFrame
        _latest_dt: 최신 날짜 (center_latest_dates가 없을 때 사용)
        centers: 대상 센터 리스트
        center_latest_dates: 센터별 최신 날짜 매핑 (선택)

    Returns:
        resource_code와 센터별 재고자산 컬럼을 가진 DataFrame
    """
    if snap_raw is None or snap_raw.empty:
        return pd.DataFrame(
            columns=["resource_code"] + [f"{c}_재고자산" for c in centers]
        )
    ...
    if normalized_center_dates:
        relevant_dates = list(
            {dt for dt in normalized_center_dates.values() if pd.notna(dt)}
        )
    ...
    for ct in centers:
        ...
        if center_sub.empty:
            continue
        qty = (
            pd.to_numeric(center_sub[src_col], errors="coerce").fillna(0).clip(lower=0)
        )
        if qty.empty:
            continue
    ...
    for ct, g in cost_cols.items():
        base = base.merge(
            g.rename(columns={col_sku: "resource_code"}), on="resource_code", how="left"
        )
